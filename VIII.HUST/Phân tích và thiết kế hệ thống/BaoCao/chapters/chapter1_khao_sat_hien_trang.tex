...
Chương 1: bối cảnh, phạm vi, yêu cầu. Các chương sau theo quy trình: \textbf{Nghiệp vụ} $\rightarrow$ \textbf{UC tổng quát} $\rightarrow$ \textbf{UC phân rã} $\rightarrow$ \textbf{Đặc tả UC} $\rightarrow$ \textbf{Activity} $\rightarrow$ \textbf{Sequence} $\rightarrow$ \textbf{Class} $\rightarrow$ CSDL/Giao diện.
